...
- `make ubuntu-setup-script` — создать исполняемый скрипт `ubuntu22_setup.sh` для ручной настройки Ubuntu 22.
...
Чтобы получить копию скрипта под Ubuntu 22 рядом с проектом:
